...
def score(run_dir, stats, files, dev, frames, batch):
    models, epochs, paths = load_models(run_dir, "", dev)
...
def main():
    ...
    result = {}
    for tag, run_dir in (("baseline_info_form", args.baseline_dir), ("full_field_ablation", args.fullfield_dir)):
        print(f"\n=== {tag}  ({run_dir}) ===")
        r = score(run_dir, stats, files, dev, args.frames, args.batch)
        result[tag] = r
        print(f"  output averaging over {len(r['checkpoints'])} checkpoint(s): epochs {r['epochs']}")
