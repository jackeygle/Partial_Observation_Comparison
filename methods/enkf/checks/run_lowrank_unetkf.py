...
from methods.enkf.surrogate.model import (CLIP_HI, CLIP_LO, EMPTY_DENSITY,
...
def load_models(checkpoint, device):
    ...
    cov.load_state_dict(ckpt["model"])
    cov.eval()
    ...
    # covariance model. Older frozen-mean checkpoints intentionally do not.
...
def run_file(path, mean_net, cov_net, static_mask, device, frames, covariance_mode,
    ...
    with torch.inference_mode():
        for t in range(1, total):
            ...
            factor, diagonal = cov_net(state, mean, static_mask)
